// src/modules/buitendienst/MijnPlanningPage.jsx
...
export default function MijnPlanningPage() {
  ...
  // artikelen ophalen (geen kolomnamen in query – tabel heeft omschrijving, geen naam)
  useEffect(() => {
    if (!user) {
      setArtikelen([]);
      setArtikelenErr("");
    ...
    supabase
      ...
      .then(({ data, error }) => {
        ...
        // Sorteer client-side op omschrijving (geen .order() naar Supabase)
  ...
  const filteredArtikelen = useMemo(() => {
    ...
    return (artikelen || []).filter((a) => {
      const omschrijving = String(a?.omschrijving || "").toLowerCase();
      const nr = String(a?.artikelnummer ?? "").toLowerCase();
      return omschrijving.includes(q) || nr.includes(q);
    });
  ...
  /**
   * Zelfde route als OutlookPlanningBoard/WerkbonEditModal (useWerkbonnen.replaceWerkbonMaterieel):
   * delete werkbon_materieel op werkbon_id, dan insert regels { werkbon_id, artikel_id, aantal }.
   * @param {string} werkbonId
   * @param {Array} materialenArray - form.materialen (expliciet meegeven i.v.m. closure bij async)
   */
  async function syncMaterialenToWerkbonMaterieel(werkbonId, materialenArray) {
    ...
    if (items.length > 0 && valid.length === 0) {
      console.warn("syncMaterialenToWerkbonMaterieel: geen geldige regels (artikel_id en aantal > 0 vereist)", items);
      throw new Error("Geen geldige materialen: controleer dat elk regel een artikel en aantal > 0 heeft.");
    }
    const { error: delError } = await supabase.from("werkbon_materieel").delete().eq("werkbon_id", String(werkbonId));
    if (delError) throw new Error(`Materialen wissen: ${delError.message}`);
    if (valid.length === 0) return;
    const rows = valid.map((m) => ({
      werkbon_id: String(werkbonId),
      artikel_id: String(m.artikel_id),
      aantal: Number(m.aantal ?? m.qty ?? 1) || 1,
    }));
    if (import.meta.env.DEV) console.log("syncMaterialenToWerkbonMaterieel: inserting", rows.length, "rows", { werkbonId, rows });
    const { data: inserted, error: insError } = await supabase.from("werkbon_materieel").insert(rows).select("id");
    if (insError) throw new Error(`Materialen opslaan: ${insError.message}`);
    if (import.meta.env.DEV) console.log("syncMaterialenToWerkbonMaterieel: inserted", inserted?.length ?? 0, "rows");
    if (import.meta.env.DEV && inserted?.length !== rows.length) {
      console.warn("syncMaterialenToWerkbonMaterieel: insert count mismatch", { expected: rows.length, got: inserted?.length });
    }
  }
  ...
  async function handleSave() {
    ...
    try {
      ...
      await syncMaterialenToWerkbonMaterieel(selectedBon.id, form.materialen);
                ...
                >
                  ...
                  {filteredArtikelen.map((a) => (
                    <option key={a.id} value={a.id}>
                      {a.omschrijving || "—"} {a.artikelnummer != null ? `(${a.artikelnummer})` : ""}
                    </option>
                  ))}
                </select>
